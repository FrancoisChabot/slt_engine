...
Dist DefaultRectScorer(iVec2 const& src, iVec2 const& dst) {
...
class RectPacker2D {
 public:
  ...
  explicit RectPacker2D(iVec2 bin_size, Alloc const& alloc = Alloc())
      : free_rects_(alloc) {
  ...
  bool pack(iVec2 rect, Result& result,
            RectScorer scorer = DefaultRectScorer<Dist>) {
    ...
    bool right_added =
        addFreeRect({destination.pos[0] + rect[0], destination.pos[1]},
                    {rem_w, destination.size[1]});

    bool bottom_added =
        addFreeRect({destination.pos[0], destination.pos[1] + rect[1]},
  ...
  // any
  // other existing free rect. returns wether insertion was successful.
  bool addFreeRect(iVec2 const& pos, iVec2 const& size) {
 ...
 private:
  struct FreeRect {
    ...
    // Two rects with bi-directional linking counterpart have an overlapping
    // section
  ...
  using FreeRectAllocator =
      typename std::allocator_traits<Alloc>::template rebind_alloc<FreeRect>;
  ...
  std::tuple<FreeRectIte, bool> chooseSlot(iVec2 const& rect,
                                           RectScorer scorer) {
    ...
    for (auto ite = free_rects_.begin(); ite != free_rects_.end(); ++ite) {
      ...
      Dist score =
          fits(rect, ite->size) ? scorer(rect, ite->size) : non_fit_score;
      Dist flipped_score = fits(flipped_rect, ite->size)
  ...
  static bool fits(iVec2 src, iVec2 dst) {
